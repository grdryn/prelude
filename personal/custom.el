(custom-set-variables
 ...
 '(forge-alist
   '(("github.com" "api.github.com" "github.com" forge-github-repository)
     ...
     ("gitlab.gnome.org" "gitlab.gnome.org/api/v4" "gitlab.gnome.org"
      forge-gitlab-repository)
     ("gitlab.cee.redhat.com" "gitlab.cee.redhat.com/api/v4"
      "gitlab.cee.redhat.com" forge-gitlab-repository)
   ...
   '(("A" . "Agendas")
      ...
      ((tags-todo "URGENT"
       ...
       (tags-todo "Depth=\"Deep\"/NEXT"
                  ((org-agenda-overriding-header
                    "Next Actions requiring deep work")))
       (agenda ""
     ...
     ("AM" "Monthly overview" agenda ""
      ((org-agenda-overriding-header "Monthly overview")
       (org-agenda-span 'month) (org-deadline-warning-days 0)
 ...
 '(org-capture-templates
   '(("G" "Define a goal" entry
      ...
      (file "~/org/goal.template") :empty-lines-after 2)
     ("N" "NEXT entry" entry
      (file+headline "~/org/capture.org" "Capture")
      (file "~/org/next.template") :empty-lines-before 1)
     ("T" "TODO entry" entry
      (file+headline "~/org/capture.org" "Capture")
      (file "~/org/todo.template") :empty-lines-before 1)
     ("W" "WAITING entry" entry
      (file+headline "~/org/capture.org" "Capture")
      (file "~/org/waiting.template") :empty-lines-before 1)
     ("S" "SOMEDAY entry" entry
      (file+headline "~/org/capture.org" "Capture")
      (file "~/org/someday.template") :empty-lines-before 1)
     ("P" "PROJ entry" entry
      (file+headline "~/org/capture.org" "Capture")
      (file "~/org/proj.template") :empty-lines-before 1)
     ("B" "Book list" entry
      (file+headline "~/org/personal.org" "Reading list")
      (file "~/org/book.template") :empty-lines-after 2)
     ("t" "TV watch list" entry
      (file+headline "~/org/personal.org" "Watch list")
      (file "~/org/tv.template") :empty-lines-after 2)
     ("f" "Film watch list" entry
      (file+headline "~/org/personal.org" "Watch list")
      (file "~/org/film.template") :empty-lines-after 2)
     ("p" "Create a daily plan")
     ("pP" "Daily plan personal" plain
      (file+olp+datetree "~/org/plan-free.org")
      (file "~/org/tpl-dailyplan.txt") :immediate-finish t)
     ...
     ("jW" "Journal entry work " entry
      (file+olp+datetree "~/org/journal-work.org") "** %^{Heading}")
     ("c" "Protocol" entry (file+headline "~/org/capture.org" "Inbox")
 ...
 '(package-selected-packages
   '(protobuf-mode vertico nix-mode helm-rg helm-lsp forge emacsql-sqlite
                   poly-erb keycast lsp-metals fira-code-mode
                   dhall-mode csv-mode pcap-mode transient scala-mode
                   cython-mode lsp-python-ms lsp-java ron-mode lsp-ui
                   company-lsp nlinum ag zop-to-char zenburn-theme
                   yari which-key web-mode volatile-highlights
                   undo-tree tide super-save smartrep smartparens
                   scss-mode rainbow-mode rainbow-delimiters
                   poly-ansible pinentry operate-on-number move-text
                   json-mode js2-mode inf-ruby imenu-anywhere hl-todo
                   helm-projectile helm-descbinds haskell-mode
                   guru-mode groovy-mode gotest golint go-snippets
                   go-projectile gnu-elpa-keyring-update
                   git-timemachine gist geiser flycheck-rust
                   expand-region exec-path-from-shell elisp-slime-nav
                   editorconfig easy-kill dockerfile-mode
                   discover-my-major diminish diff-hl dap-mode crux
                   counsel company-go company-ansible company-anaconda
                   cargo browse-kill-ring beacon archive-rpm anzu
                   ansible-vault adoc-mode))
...
(custom-set-faces
 ...
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(default ((t (:inherit nil :extend nil :stipple nil :background "#3F3F3F" :foreground "#DCDCCC" :inverse-video nil :box nil :strike-through nil :overline nil :underline nil :slant normal :weight bold :height 140 :width normal :foundry "nil" :family "FiraCode Nerd Font Propo")))))
